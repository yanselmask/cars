    ...
    <section class="d-flex align-items-center position-relative min-vh-100 py-5">
        ...
        <div class="container d-flex justify-content-center position-relative zindex-5 text-center">
            <div class="col-lg-8 col-md-10 col-12 px-0">
                ...
                <p class="h2 mb-lg-5 mb-4 pb-2 text-light">{{ __('The requested page doesn’t exist') }}</p>
                <a class="btn btn-lg btn-primary w-sm-auto w-100 mb-3 me-sm-4"
                    href="{{route('home')}}">{{ __('Go to homepage') }}</a>
                <a class="btn btn-lg btn-outline-light w-sm-auto w-100 mb-3"
                    href="{{config('app.url') . '/' . config('listing.path_page') . '/contact-us'}}">{{ __('Contact us') }}</a>
            </div>
        </div>
    </section>
    @push('js-libs')
