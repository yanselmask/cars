<li class="nav-item @if($item['children']) dropdown @endif
       @switch($item['type'])
           @case('page')
           {{request()->fullUrl() == route('pages.show', $item['data']['page']) ? 'active' : ''}}
       @break
       @case('post')
           {{request()->fullUrl() == route('blog.show', $item['data']['post']) ? 'active' : ''}}
       @break
       @default
           @if(request()->getPathInfo() == $item['data']['url']) active @endif
        @endswitch
">
    <a class="nav-link @if($item['children']) dropdown-toggle @endif @isset($item['data']['classes']) {{$item['data']['classes']}} @endisset"
       target="{{$item['data']['target']}}"
       @switch($item['type'])
           @case('page')
               href="{{route('pages.show', $item['data']['page'])}}"
       @break
       @case('post')
           href="{{route('blog.show', $item['data']['post'])}}"
       @break
       @default
           href="{{$item['data']['url']}}"
        @endswitch
