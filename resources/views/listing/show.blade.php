        ...
        <div class="row">
            <div class="col-md-7">
                ...
                <div class="accordion accordion-light" id="features">
                    @foreach (\App\Enums\FeatureType::labels() as $k => $featureType)
                        <div class="accordion-item">
                            <h2 class="accordion-header" id="heading{{ $featureType }}">
                                <button class="accordion-button @if (config('listing.listing_feature_type_showed') !== \App\Enums\FeatureType::from($k)) collapsed @endif"
                                    type="button" data-bs-toggle="collapse" data-bs-target="#{{ $featureType }}"
                                    aria-expanded="@if (config('listing.listing_feature_type_showed') == \App\Enums\FeatureType::from($k)) true @else false @endif"
                                    aria-controls="{{ $featureType }}">{{ $featureType }}</button>
                            </h2>
                            <div class="accordion-collapse collapse @if (config('listing.listing_feature_type_showed') == \App\Enums\FeatureType::from($k)) show @endif"
